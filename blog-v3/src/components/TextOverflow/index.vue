...
const offset = ref(0); // 元素宽度
const slotBoxWidth = ref(0); // 插槽的宽度
const textBoxWidth = ref(0); // 文字宽度
...
const realText = computed(() => {
  ...
  if (isCutOut && showSlotNode.value && !expanded.value) {
...
<template>
  <div class="text-ellipsis" ref="textOverflowRef" :style="boxStyle">
